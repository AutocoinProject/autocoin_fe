...
export default function BinanceBotPage() {
  ...
  const [transactions, setTransactions] = useState([
    { id: 1, type: 'BUY', coin: 'BTC', amount: 0.05, price: 58432, timestamp: new Date().toISOString() },
    { id: 2, type: 'SELL', coin: 'ETH', amount: 1.2, price: 2890, timestamp: new Date().toISOString() },
  ]);
  
  const [walletBalance, setWalletBalance] = useState([
    { coin: 'BTC', amount: 0.25, valueUSD: 12500 },
    { coin: 'ETH', amount: 4.5, valueUSD: 13000 },
    { coin: 'USDT', amount: 5000, valueUSD: 5000 },
  ]);
  
  // 사용 가능한 코인 및 전략 목록
  const availableCoins = ['BTC', 'ETH', 'BNB', 'ADA', 'XRP', 'SOL', 'DOT', 'DOGE', 'USDC'];
  const availableStrategies = ['RSI', 'MACD', 'Moving Average', 'Bollinger Bands', 'AI Strategy'];
  ...
  const handleAutomaticCoinSelection = () => {
    ...
    setSelectedCoins(['BTC', 'ETH', 'BNB', 'ADA', 'XRP']);
  };
  
  return (
    <div className="p-6 max-w-full">
      <h1 className="text-2xl font-bold mb-6">Binance Trading Bot</h1>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {availableStrategies.map((strategy) => (
            ...
            >
              {strategy}
              {strategy === 'AI Strategy' && <span className="ml-2 text-xs">(Coming Soon)</span>}
            </button>
          ))}
        </div>
      </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">가격</th>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {transactions.map((tx) => (
                  <tr key={tx.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">${tx.price.toLocaleString()}</td>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">코인</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">수량</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">USD 가치</th>
                </tr>
              </thead>
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {walletBalance.map((balance, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">${balance.valueUSD.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
